...
<div class="navbar navbar-inverse navbar-fixed-left">
  ...
  <ul class="nav navbar-nav">
   ...
   @if ( count($products) > 1 )
     ...
     <form class="form-group" action="{{ url('/admin/shop/show_product') }}" method="post">
       {{ csrf_field() }}
          <!-- <strong>{{ $product->productname }}</strong> -->
            ...
            <div class="panel panel-default">
              <div class="panel-heading clearfix ">
                <h4 class="panel-title pull-left" style="padding-top: 7.5px;">Product Description</h4>
                    <a  href = "{{url('/admin/cart')}}" type="submit"  class="btn btn-primary pull-right" role = "button">
                        View Cart
                    </a>
              </div>
                ...
                <div style="color:black" class="panel-body">
                  ...
                  <?php $prod = DB::table('products')->where('id', $product_id)->get()->first(); ?>
                   ...
                   @if($prod->quantity > 0)
                     <form class="form-group" action="{{url('/admin/cart_add')}}" method="post">
                       {{ csrf_field() }}
                          <!-- <strong>{{ $product->productname }}</strong> -->
                          <div class="row">
                            <br>
